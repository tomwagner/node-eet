...
test('serializeKontrolniKody', t => {
	...
	t.is(helpers.serializeKontrolniKody(PRIVATE_KEY, data), expected);
...
test('eetGenerateSecurityCodes', t => {
	...
	t.deepEqual(eet.eetGenerateSecurityCodes(data, PRIVATE_KEY), expected);
...
<eet:Odpoved>
	...
	t.deepEqual(await helpers.parseResponseXML(xml), expected);
...
<eet:Odpoved>
	...
	const parsed = await helpers.parseResponseXML(xml);
...
<eet:Odpoved>
	...
	const error = await t.throwsAsync(helpers.parseResponseXML(xml));
