...
import { parseResponseXML, serializeKontrolniKody } from '../src/helpers';
import { eetGenerateSecurityCodes } from '../src';
...
test('serializeKontrolniKody', t => {
	...
	t.is(serializeKontrolniKody(PRIVATE_KEY, data), expected);
...
<eet:Odpoved>
	...
	t.deepEqual(await parseResponseXML(xml), expected);

});

test('parseResponseXML warning', async t => {
...
<eet:Odpoved>
	...
	const parsed = await parseResponseXML(xml);
...
<eet:Odpoved>
	...
	const error = await t.throwsAsync(parseResponseXML(xml));
...
test('request wrong certificate', async t => {
	...
	const error = await t.throwsAsync(eetSend(data, options));
	t.assert(error instanceof errors.ResponseServerError);
	t.is(error.code, '4');
...
test('getWarnings wrong datTrzby', async t => {
	...
	t.is(warnings.code, '5');
	t.log('Warning:', warnings.message);
